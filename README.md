...
Nearly all the functions you write are pure, so the computational pieces of your app can be
described, understood and tested independently.

Composition of these computational parts happens via reactive data flows - a dynamic,
...
re-frame is impressively buzzword compliant:  it has FRP-nature,
...
Its a mashup of existing, emerging ideas. We're releasing it because we wished
we'd found this sort of an explanation and implementation when we started.
...
"Derived Data All The Way Down" (an infinite loop of Derived Data). More explanation on this soon.
...
for computation. To  be that another way, they can create
...
We could write the subscription-handler:
...
part of `app-db` may have changed. We don't want to perform a computationally expensive re-sort
each time something unrelated changes.

Luckily, we can easily fix that up by slightly changing our subscription function so that it chains `reactions`:
...
The 1st and 2nd reactions just extract from `db`.  The 3rd one does the expensive
...
The old Signal value is compared with the new Signal value via `identical?`. Because we're
...
There's a single `dispatch` function in the entire framework, and it takes one parameter: the event (vector) to be
...
The consumer on the end of the conveyor is a `router` which will organise to for that
...
This is utterly perfect for debugging, assuming you are in a position to capture
