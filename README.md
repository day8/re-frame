...
Along the way, I'll be using [Reagent] at an intermediate to advanced level.  So, before you go any further here, be sure you have done the [Introductory Tutorial](http://reagent-project.github.io/), and that you understand resources like, say,   
...
From a ClojureScript perspective, the purpose of any atom is to hold mutable data.  From a re-frame perspective, we'll tweak that paradigm ever so slightly and **view a `ratom` as being a value that changes over time.**  Seems like a subtle distinction, I know, but becaause of it re-frame sees a `ratom` as an FRP Signal. If you don't know what that means then please [pause and read this](http://elm-lang.org/learn/What-is-FRP.elm).
...
So, via `ratoms` and `reactions`,  values 'flow' into computations and out again, and then into other computations, etc.  Data flows through the Signal graph. But our graph will be without cycles, because cycles are bad! We want unidirectional dataflow.
