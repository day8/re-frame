...
- Fix execution order of global interceptor functions. So that, for example,
  it is possible to inspect the `:db` result of an event handler and inject
  `:effects` for `do-fx` in an `:after` of a global interceptor.
