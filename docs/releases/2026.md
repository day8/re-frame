...
- `re-frame.macros` — drop-in macro replacements/mirror for certain
  `re-frame.core` functions that capture call-site `{:file :line}`
  information to facilitate tracing and, as a result, tooling like
  `re-frame-10x` and `re-frame-pair`.
  - functions mirrored — `re-frame.macros/dispatch`, `dispatch-sync`,
    `subscribe`, `reg-event-db`, `reg-event-fx`, `reg-event-ctx`,
    `reg-sub`, `reg-fx`
  - to use, swap `re-frame.core` for `re-frame.macros` in your
    `:require`:
